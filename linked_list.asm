...
_start:
        ...
        cmp rax, 0              ;check if mmap failed
        jl exit
        ...
        add qword [rsp], struct_size	;advance tail
...
;;         rax            rdi              rsi
;;add_node(char new_char, node* prev_node, node* new_node)
add_node:
        mov [rsi+char_offset], al     ;add new_char to our node
        mov qword [rsi+ptr_offset], 0 ;set our next_ptr in new_node to null

        test rdi, rdi
        jz skip_prev		;if prev pointer is 0 (null) then don't get previous pointer (for head)
        mov [rdi+ptr_offset], rsi	;move address of new_node to prev_node ptr field
skip_prev:
        ret
...
traverse_list:
        ...
        mov al, byte [rax+char_offset] ;dereference struct to get char
        call print_char
        pop rax			;restore pointer to struct
        mov rax, [rax+ptr_offset] ;store next node in rax to prepare to jump
        test rax, rax             ;mov's don't set zf...
        jnz traverse_list	;if the next node is 0 (null) then return, otherwise repeat the steps
        ret
