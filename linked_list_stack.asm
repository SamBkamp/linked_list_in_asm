...
_start:
	;***** This implementation contains a node on the stack ****
	...
	;node on stack
	mov rax, 0x44		;D
	mov rdi, qword[rsp]	;get tail (bottom of stack)
	sub rsp, 9		;make space on stack for new node
	mov rsi, rsp		;pass our stack pointer
	call add_node
	
	mov rax, qword [rsp+17] 		;get bottom of heap address (which should be the head) (9 + 8 [struct node + ptr] = 17)
	call traverse_list	
	
	mov rax, 0xb		;munmap
	mov rax, qword [rsp+17] 		;get bottom of heap address (which should be the head)
	mov rsi, 4096
	syscall

	add rsp, 25		;realign stack - 2 ptr (head and tail) + one struct = 25 (8+8+9)
